# PyOxidizer configuration for building a standalone `ppt` executable.
#
# This produces a self-contained binary that can run `ppt` without requiring a
...
def make_exe():
    ...
    # Prefer in-memory resources so the binary is truly self contained.
    ...
    # Start as if running `python -m ppt`.
    python_config.run_module = "ppt"
    ...
    # Add our sources from the repo `src/` directory.
    exe.add_python_resources(exe.read_package_root(path="src", packages=["ppt"]))
...
def make_install(exe):
    ...
    # Release asset layout expected by `ppt`: `bin/ppt` should exist.
    # `add_python_resource()` takes a destination directory and uses the
    # executable's name as the filename.
